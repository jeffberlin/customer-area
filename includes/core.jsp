...
<c:set var="sessionid" value="${cookie['BMTMicro.Customers.SessionID'].value}" />
<c:set var="lastlogin" value="${URLDecoder.decode(cookie['BMTMicro.Customers.LastLogin'].value,'UTF-8')}" />
...
<c:set var="fromOffset" value="${cookie['BMTMicro.Customers.FromDate'].value}" /><c:if test="${fromOffset == null}"><c:set var="fromOffset" value="6"/></c:if>
...
<c:set var="flags" value="${cookie['BMTMicro.Customers.Flags'].value}" /><c:if test="${flags == null}"><c:set var="flags" value="0"/></c:if><c:if test="${flags == -1}"><c:set var="flags" value="4294967295"/></c:if>
<c:set var="allowTestOrders" value="0" /><c:if test="${(flags%2)>=1}" ><c:set var="allowTestOrders" value="1" /></c:if>
<c:set var="allowChanges" value="0" /><c:if test="${((flags/2)%2)>=1}" ><c:set var="allowChanges" value="1" /></c:if>
<c:set var="allowResend" value="0" /><c:if test="${((flags/4)%2)>=1}" ><c:set var="allowResend" value="1" /></c:if>
